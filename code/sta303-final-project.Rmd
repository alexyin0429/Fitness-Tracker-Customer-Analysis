...
This report is responsible for analyzing two questions regarding the Mingar wearable fitness tracker. The first question is about the differences between customers who purchase Active or Advance products and customers who purchase traditional products such as devices from the Run product line. The second question is about whether darker skin color of customers affects the performance of the sleep score function, or there some additional variables will affect it as well. In terms of the first questions, we have two findings:
...
- Since we are interested in the relationship between customers’ skin color and the number of quality flags that occurred during the sleep session, Figure 1 showed that darker-skinned customers had the largest range of the occurrence of the number of flags per minute with respect to a sleep session.
...
```{r,fig.width = 4, fig.height = 2.5,fig.cap="The histogram about the number of quality flags per minute during the sleep session in different skin color customers",echo=FALSE,include=TRUE}
...
After Mingar released the Active and Advance product line, there are an increasing number of customers who chose to own one of these products. Since these newly released products are extremely popular and attracts lots of new customers, it is important to understand the factors or characteristics that differentiate the new customers from the traditional ones.

In this report, we will investigate this question by analyzing the preference with respects to device functions and price of new customers and traditional customers directly from data and by building a statistical model (e.g. Generalized Linear Mixed Model). 
...
The rest of the report of each question is organized as follows. In __Data__ section, we discussed how we collected our data from various sources and the cleaning process. Also, we displayed the results of exploratory data analysis in this section. In __Method__ section, we discussed the methods and model assumptions as well as how to assess them to get the best results. Then, in __Result__ section, for first research question, we showed the results of analyzing the differences in customers' characteristics directly from data and models as well as the conclusion we drew from these two methods. And for second research question, the model parameters were analyzed and discussed according to the research question. Lastly, __Discussion__ section summarized our finding to the question, as well as pointing out the strengths and limitations.
...
Since the subject of our study is the device’s sleep function aspect, we would focus on the provided dataset containing customers’ sleep data and want more variables that are related to customers and devices that are to be studied. Both the datasets of customers’ personal information and devices purchased by customers were merged with customers' sleep information by customer ID and then combined with device information by device ID. All combined data were stored in a new dataset named *merged customer sleep* and unrelated variables postal code, customer's pronouns for social profile, device ID, and release date of the particular device were deleted. 

In addition, removed missing values in the customer’s sex (sex) based on the dataset *merged customer sleep* and only included males and females since the proportion of intersex was very small overall relatively. Then a new variable representing the customer’s age (age) was created by calculating their age from their date of birth to the date of today and rounding it to the nearest year. Also, deleted the variable of the customer’s date of birth (dob). Created a new variable for the customer’s skin color (skinColor) to represent the corresponding skin tone for customers' emoji modifier when using the chat and react features of the social component of the company’s app. Specifically, categorized skin color as medium, medium-light, light, medium-dark and dark when the emoji modifier was U+1F3FD, U+1F3FC, U+1F3FB, U+1F3FE, U+1F3FF respectively based on the chart of Full Emoji Modifier Sequences[17]. If the value of the customer’s emoji modifier was missing, classified it as unspecified since these customers used the default yellow color as their emoji modifer. After that, deleted the emoji modifier variable. Moreover, rescaled customers’ age into the range from 0 to 1 by calculating the difference between age and minimum age level in the dataset and then over the difference between maximum age and minimum age in the dataset. This would allow fitting any model more efficiently and conveniently later on. Besides, created a variable (flagProp) representing the number of quality flags per unit(minute) of sleep session. And created a variable (flagMean) representing the mean of the number of quality flags per minute for each age level. These two variables would be used in data exploration to construct figures or tables.

In the end, the *merged customer sleep* dataset that has been cleaned up was output and read in as the *sleep* dataset we would explore and analyze later on.

### Variables of Interest

- Skin color (skinColor): the skin color of a customer, transferred from the code for skin tone modifier for emojis when using the chat and react features of the social component of the company’s app.
- Flags (flags): the number of times occurring a quality flag during a customer’s sleep session. This might due to missing data, sensor error or other data quality issues of the device. This would be the variable we would use to assess the sleep score function of a device.
- Duration (duration): the length of sleep session in minutes.
- Age (age): the age of a customer.
- Sex (sex): the biological sex of a customer.
- Customer ID (cust_id): unique ID for each customer.
- Date (date): the starting date of the sleep session.
- Device name (device_name): Name of device type.
- Line (line): the product line of customer purchased device belongs to.
- Scaled age (ageScale): the scaled customer’s age, ranges from 0 to 1.
- Flag Proportion (FlagProp): the number of times the quality flag occurred per minute during a sleep session. This variable measures the number of quality flags using the same unit (minute) as observations may have different durations of sleep sessions.
- Flag Mean (FlagMean): the number of times the quality flag occurred per minute during a sleep session for each age level. 
...
```{r,fig.cap="The histogram about the number of quality flags per minute during the sleep session in different skin color customers",echo=FALSE,include=TRUE}
sleep_data %>% 
  ggplot(aes(x = flagProp)) + 
  geom_histogram(bins = 20, color="darkblue", fill="lightblue") + 
  xlab("the number of quality flags occurring per minute") + 
  ylab("frequency of the number of quality flags per minute") +
  facet_wrap(~skinColor) + 
  theme_grey()
```

From Figure 1, each of the six plots has only one mode, and the mode of the number of flags per minute for the devices purchased by customers belonging to light skin, medium-light, medium, and unspecified skin was less than or near 0.01, respectively. However, the mode of the number of flags per minute for the medium-dark skinned customer graph was close to 0.02, which was slightly lower than the one for dark-skinned customers. All the plots were almost normally distributed. However, the range of the number of flags per minute on dark-skinned customers were the largest, from about 0.007 to 0.063. Meanwhile, the range for medium-dark-skinned customers was about 0 to 0.05. The range of the number of flags per minute on light-skinned customers is the smallest, from roughly 0 to 0.015, and the other skin color customers were around 0 to 0.028.
...
knitr::kable(sleep_data %>% group_by(skinColor) %>% 
             ...
             cutormers with different skin colors", 
             col.names = c("customers' skin color", 
                           "average flags per minute"))
```
Table 1 summarizes the average probability of flags per minute for the devices used by customers with different skin colors. It shows that dark-skinned customers have the highest number of flags per minute on average(0.0334) which means their devices have a higher probability of occurring flags during sleep sessions. This might due to missing data, sensor error, or other data quality issues. Additionally, the number of flags per minute on average for medium-dark skinned customers is approximately 0.013 lower than dark-skinned customers. Meanwhile, the average number of flags per minute occurring on light-skinned customers is the smallest which is 0.0031 among six types.

Therefore, according to Figure 1 and Table 1, we propose that the number of quality flags that occurred for a device during the sleep session would vary by customer's skin color. 


```{r,fig.cap="The scatterplot about the average number of quality flags per minute during the sleep session for customers at different age levels",fig.height=3, echo=FALSE,include=TRUE}
sleep_data %>% 
  ggplot(aes(x = age, y = flagMean)) + 
  geom_point(shape=20, color="blue", size=2) + 
  xlab("the customer's age") + 
  ylab("average quality flags per minute") + 
  theme_bw()
```

Then, we propose that the device's sleep score function may be influenced by the customer's age. Figure 2 shows the distribution of customer age and the average number of flags per unit of sleep session(minute). The average number of flags per minute for customers aged less than 25 years fluctuated in a wide range, from about 0.005 to 0.02. For most of the points between customers ages 25 and 75, the average number of flags per minute fluctuates in a range of about 0.007, which is a really small difference. However, customers over 75 years had the highest fluctuations in their average number of flags per minute, reaching a maximum of about 0.037 and a minimum of less than 0.003. Overall, the average number of flags occurring per minute detected by the device decreased slightly when customers’ age increased. Thus, customers' age would be a factor that leads to different numbers of quality flags during a sleep session.  

```{r, echo=FALSE, include=TRUE}
knitr::kable(sleep_data %>% distinct(cust_id, .keep_all = TRUE) %>% 
             group_by(device_name) %>% summarise(count = n()) %>% 
             arrange(desc(count)),
             col.names = c("device type name","total number of devices used by customers"),
             align = c("c","c"),
             caption = "Summary of the total number of each device used by 
             the customers")
```

Apart from that, we know the quality flag occurred due to any missing data, or unusual data resulting from a sensor error or other data quality issue. Then, we propose that the sleep score function may be affected by the device type the customer used. Table 2 summarizes the number of devices purchased by customers. We can see that the device with the highest number of users is Advance 2, with 392 users. The number of people using Run ON and Run BE is close to 200 respectively, and Advance users and Active Alpha users are both near 100. However, less or equal than 5 users use Run 875 X, Run 7, Run 7 Plus or iDOL, especially Run 7 has the lowest number of users, only 1. Thus, Advance 2 is the most popular device, while Run 7 is the least popular among them.

```{r,fig.cap="Boxplot about the number of flags per minute during the sleep session for different device types", fig.height = 4, echo=FALSE,include=TRUE}
sleep_data %>% 
  ggplot(aes(x = device_name, y = flagProp)) + 
  geom_boxplot(color = "blue", fill = "lightblue") + 
  coord_flip() + 
  xlab("average quality flags per minute") + 
  ylab("the device type name") + 
  theme_bw()
```

Figure 3 presents the number of flags per minute during the sleep session for different devices. All devices have the minimum number of flags per minute, which is 0, and a similar median of around 0.006, except for device Run 7, which has a median and minimum of around 0.032 and 0.012 respectively since the number of users is too small. Patterns of device Active Alpha, Advance, and Run BE are similar with maximum, median, minimum, and the middle 50% of the number of flags per sleep session in minute(IQR), while Run BE has the largest outliers(approximately 0.072) among them. Moreover, iDOL and Run 7 Plus have a similar pattern as well although the IDOL occurs the number of flags per minute is slightly higher. Thus, it is likely that different types of devices would have different numbers of quality flags occurring during sleep sessions. 

Based on all of the figures and tables discussed above, the customer’s skin color, age, and the type of device they used would all have an impact on the number of quality flags during the sleep session. These three variables could be used as predictors when fitting the model later. 

## Method

For this research question, we are interested in investigating the factors that would affect the performance of the device’s sleep score function. More importantly, we would like to find out if there is a relationship between the device's performance of the sleep score function and the skin color of the customer. According to the variables of interest from the sleep dataset discussed in the previous Data section, the number of times the quality flag occurred during the sleep session for a customer would be a good choice to assess the sleep score function for the device. 

To discover the association between sleep score performance of the device and customer’s skin color, we would fit a model with the number of times the quality flag occurred during the sleep session as the response variable and the customer’s skin color, age, and the device type name of the type of device they used as three predictors. This should be a generalized linear model (GLM) as the response variable is a count data. However, we observed that for one customer, we have the number of times the quality flag occurred during multiple sleep sessions, which implies that these sleep-session observations are dependent on each customer. In this case, we chose to use a generalized linear mixed model (GLMM) with the customer ID representing a specific customer as a random effect. Specifically, we would treat customer ID as a random intercept because, for each customer, the number of times the quality flag occurred should be different from the intercept. 

Since the number of quality flags is a count data, then the response distribution would be the Poisson distribution, which is the family of distribution we used when fitting the generalized linear mixed model. Another thing to notice is that for each observation, the duration of the sleep sessions is quite different from each other when accounting for the number of quality flags. It is necessary to consider it as an offset term when fitting the model with Poisson regression. Also, we use the scaled age of customers instead of age so that it would be more efficient and computationally convenient when fitting the model. 
...
- $flags_{ij}$ is the number of times there was a quality flag during $i$th sleep session for $j$th customer, which is the response variable and follows a Poisson distribution with $\lambda = \mu_{ij}$.
- $log(\mu_{ij})$ is the log count of the number of quality flags that occurred during $i$th sleep session for $j$th customer. It uses the log link function for transformation. $\mu_{ij}$ is the number of quality flags that occurred during $i$th sleep session for $j$th customer. 
- $U_j$ is the **random effect** for $j$th customer and $U_j$~$N(0, \sigma^2)$
- $log(duration)$ is the offset term, which has a fixed coefficient estimate of 1. 
- $\beta_0 + \beta_1skinColor_{ij} + \beta_2ageScale_{ij} + \beta_3device$_$name_{ij}$ are the **fixed effect terms**. 
- $skinColor_{ij}$ is a categorical variable representing the skin color of $j$th customer for $i$th sleep session.
- $ageScale_{ij}$ is a numerical variable representing the scaled age of $j$th customer for $i$th sleep session.
- $device$_$name$ is a numerical variable representing the name of the device type used by $j$th customer for $i$th sleep session.

We would interpret the exponential of each $\beta$ parameter as it is more meaningful.

- $\beta_0$ is the intercept term. The exponential of $\beta_0$ ($e^{\beta_0}$) represents the count of the number of occurrence of quality flags for a certain customer whose skin color is at the reference level and who is 17 years old (minimum customer's age level).

- the exponential of $\beta_1$ ($e^{\beta_1}$) represents the relative count for the group of customers with skin color at the other level (not at the reference level) and the group of customers with the skin color at the reference level, while keeping everything else fixed.

- the exponential of $\beta_2$ ($e^{\beta_2}$) represents the relative count when a customer's scaled age increases by 1, which means the scaled age increases from 17 (minimum age level) to 92 (maximum age level), while keeping everything else fixed.

- the exponential of $\beta_3$ ($e^{\beta_3}$) represents the relative count between the group of the customer's device type at the other level (not at the reference level) and the group of the customer's device's type at the reference level while keeping everything else fixed.

### Model Assumptions

When we build the generalized linear mixed model, we need to check these four model assumptions: 

- The groups for the random effect are independent, even if the observations within each group are not independent.
- The random effect follows a normal distribution.
- The random-effects errors and residual errors within groups have constant variance.
- The model is defined correctly with the appropriate link function.

### Model selection 

According to the research question, we would like to find out factors that would have an impact on the occurrence of quality flags during sleep sessions. We have proposed a generalized linear mixed model with the number of quality flags that occurred during the sleep session as the response and three predictors as the fixed effects that may be correlated to the response variable. We would then apply some model selection techniques (i.e., Hypothesis Testing, Likelihood-Ratio Test) that could be helpful for choosing the final model. 

#### Hypothesis Testing

Hypothesis Testing could statistically check whether our hypothesis meets the condition we state. It needs to state two hypotheses which are null hypothesis $H_0$ and alternative hypothesis $H_a$. Then calculate the test statistic from sample data and p-value based on the test statistic under the assumption that $H_0$ is true.

Specifically, we choose to do the hypothesis test for the coefficient of each fixed effect, $\beta_1,\beta_2,\beta_3$ to explore the value of the p-value, which is a number describing the occurrence chance of the null hypothesis test is true[11], and use the common statistical threshold (0.05) as an indicator. Therefore, for each p-value, if it is below the threshold (< 0.05), then we would have strong evidence to reject the null hypothesis that $\beta_1=0, \beta_2=0, \beta_3=0$ respectively. We can conclude that the customer’s skin color, age, and the type of the device do have an effect on the occurrence of quality flags during the sleep session, respectively. Similarly, if it is above the threshold, then we would fail to reject the null hypothesis that $\beta_1=0, \beta_2=0, \beta_3=0$ respectively. We can conclude that the customer’s skin color, age, and the type of the device do not have an effect on the occurrence of quality flags during the sleep session, respectively. In this case, we may consider not including a certain predictor in the model as it is not quite related to the response.  	

#### Likelihood-Ratio Test

The Likelihood-Ratio test is a hypothesis test, which is a statistical test of the fit between two models[10]. It can help us choose the better model between two nested models. A nested model indicates that one model includes all the components from the other model and has additional components. In other words, a relatively complex model is compared with a simpler model to see which model fits a particular data set significantly better. Additionally, the degree of freedom of the test equals the difference in the number of components between the two models. We would use Hypothesis Testing to choose the better model. The null hypothesis($H_0$) is that the simpler model fits the data as well as the complicated model. And the alternative hypothesis($H_a$) is that the complicated model fits better of the data than the simpler one. When the test statistic is large and the p-value is below the threshold, we have enough evidence to reject the null hypothesis. In other words, the complicated model is a better fit for the data than the simple one and we should consider using the complicated model. Similarly, when the p-value is above the threshold, we fail to reject the null hypothesis and we prefer the simpler model.

## Result

According to the generalized linear mixed model we proposed at the beginning of the Method section, we have the fitted model with fixed effects of a customer’s skin color, age, and their device’s type, a random intercept of a certain customer, and an offset term of the duration. 
...
```{r, echo=FALSE, include=TRUE}
knitr::kable(summary(mod1)$coefficients[, c(1, 4)],
             caption = "Key Numerical Outputs of the Fixed Effects of the 
             Fitted Model", col.names = c("Estimate", "P-value"))
```

Table 3 summarizes the estimated coefficient and p-value for each fixed effect of the fitted model. The column of the estimated coefficient (Estimate) is the $\hat \beta$ for each of the predictors of the fixed effects. For the last column of the p-value, we could use them to draw any conclusion by conducting hypothesis testing discussed in model selection process in Method section. 

Basically, we observed that the reference level for customers’ skin color is dark skin, and for the levels of skin color being light, medium, medium-dark, medium-light and unspecified, the p-values ($<2e^{-16}$) are really close to 0, which are below the threshold (0.05) we proposed for Hypothesis Testing previously. Each of these significantly small p-values implies that we have very strong evidence against the null hypothesis that the corresponding coefficient parameter is 0 ($H_0: \beta_1 = 0$), respectively. Thus, the skin color of a customer should be related to the number of times the quality flag occurred during a sleep session. We chose to keep this predictor in the model. 

Similarly, for the scaled age of a customer, the p-value (`r output_mod1$coefficients[7,4]`) is below the threshold, so we reject the null hypothesis that the coefficient parameter is 0 ($H_0: \beta_2 = 0$), which leads to the conclusion that the scaled age of a customer would have an impact on the number of times quality flag occurred during a sleep session for the customer’s device. Thus, we chose to keep this predictor in the model. 

For a customer’s device type, the reference level is the Active Alpha, and for Advance device and Run 875 X device, the p-values are `r output_mod1$coefficients[8,4]` and `r output_mod1$coefficients[14,4]`, respectively, which are both below the threshold. However, the p-values for other type of devices are all above the threshold. Thus, we fail to reject the null hypothesis that those corresponding coefficient parameter is 0, respectively. We conclude that the device type may not be quite related to the number of times the quality flag occurred during a sleep session for the customer’s device. In this case, we may consider not including this predictor in the model. Therefore, we would fit a second model, which is a reduced model from the original generalized linear mixed model. The reduced model is exactly the same as the full model, except for it does not have the predictor, the customer’s device’s type. 
...
Now, we have two nested models. The null hypothesis for the Hypothesis Testing of the Likelihood-ratio test is that the simpler model fits the data as well as the more complicated model. According to Table 4, the value in the last cell, `r result[2, 5]`, is the p-value we got under the assumption that the null hypothesis is true. Thus, we fail to reject the null hypothesis and we could conclude that the reduced model explains the data as well as the more complicated model. We preferred the reduced model as it is simpler and that model would be our final model. 
...
```{r, echo=FALSE, include=TRUE}
knitr::kable(exp_output_mod2,
             caption = "Exponential Estimated Coefficient for the Final Model", 
             col.names = "Estimate")
```

Based on the values in table 5, the final fitted generalized linear mixed model is
...
- $\hat flags_{ij}$ is the estimated number of times there was a quality flag during $i$th sleep session for $j$th customer, which is the response variable and follows a Poisson distribution with $\lambda = \hat \mu_{ij}$.
- $log(\hat\mu_{ij})$ is the expected log count of the number of quality flags that occurred during $i$th sleep session for $j$th customer. It uses the log link function for transformation. $\hat\mu_{ij}$ is the expected number of quality flags that occurred during $i$th sleep session for $j$th customer.
- $U_j$ is the **random effect** for $j$th customer and $U_j$~$N(0, \sigma^2)$
- $log(duration)$ is the offset term, which has a fixed coefficient estimate of 1. 
- $\hat\beta_0 + \hat\beta_1skinColorLight_{ij} + \hat\beta_2skinColorMedium_{ij} + \hat\beta_3skinColorMedium$-$dark_{ij} + \hat\beta_4skinColorMedium$-$light_{ij} + \hat\beta_5skinColorUnspecified_{ij} + \hat\beta_6ageScale_{ij}$ are the **fixed effect terms**. 

Since the variable of a customer's skin color is a categorical variable with 6 levels, then the model treats the customer with dark skin color as the reference level and treats the other 5 levels as five indicator variables, $skinColorLight_{ij}$, $skinColorMedium_{ij}$, $skinColorMedium$-$dark_{ij}$, $skinColorMedium$-$light_{ij}$, $skinColorUnspecified_{ij}$, whose value would be 1 if $j$th customer has that skin color for $i$th sleep session. Otherwise, the value would be 0.

- $ageScale_{ij}$ is a numerical variable representing the scaled age of $j$th customer for $i$th sleep session. It ranges from 0 to 1, calculated by subtracting the minimum customer's age level (17) from the original age of the certain customer and then dividing the result by the difference between the maximum customer's age level (92) and minimum customer's age level from the dataset.

Normally, the expected relative count is more meaningful than the expected log relative count for interpretation. Then, we applied exponential on all of the coefficient estimates and interpreted them as follows.

- $\hat\beta_0$: `r output_mod2$coeff[1,1]` is the intercept term. The exponential of $\hat\beta_0$ ($e^{\hat\beta_0}$) is `r exp(output_mod2$coeff[1,1])`, which represents the value of the expected count of the number of occurrence of quality flags for a certain customer whose skin color is dark and who is 17 years old (minimum customer's age level).

- the exponential of $\hat\beta_1$ ($e^{\hat\beta_1}$): `r exp(output_mod2$coeff[2,1])` represents the expected relative count for the group of customers with light skin color and the group of customers with dark skin color. Since the value is smaller than 1, then the number of quality flags for customers with dark skin color would be more than the number of quality flags for customers with light skin color, while keeping everything else fixed. The expected number of quality flags that would occur during a sleep session for a customer with light skin color would be `r (exp(output_mod2$coeff[2,1]))*100`% times as many as dark skin color customer while keeping everything else fixed.

The interpretation of the exponential of $\hat\beta_2$ (`r exp(output_mod2$coeff[3,1])`; medium skin color), $\hat\beta_3$ (`r exp(output_mod2$coeff[4,1])`; medium-dark skin color), $\hat\beta_4$ (`r exp(output_mod2$coeff[5,1])`; medium-light skin color), $\hat\beta_5$ (`r exp(output_mod2$coeff[6,1])`; unspecified skin color) would be similar as the above interpretation of the exponential of $\hat\beta_1$ except for different skin color levels and values. 

- the exponential of $\hat\beta_6$ ($e^{\hat\beta_6}$): `r exp(output_mod2$coeff[7,1])` represents the expected relative count when a customer's scaled age increases by 1. When the customer's scaled age increases from 17 (minimum age level) to 92 (maximum age level), the expected count of the number of quality flags would occur during a sleep session decreases by `r (1-exp(output_mod2$coeff[7,1]))*100`%, while keeping everything else fixed. In other words, when the customer's age increases by 1, the expected count of the number of quality flags would occur during a sleep session decreases by `r (1-exp((output_mod2$coeff[7,1])*(1/(92-17))))*100`%, while keeping everything else fixed.
...
- The grouping unit of this model is the customer/user, which should be independent. Since each observation of the sleep dataset represents a sleep session record from a certain user, then the observations/sleep session records from the same user would be dependent. Thus, the first assumption is satisfied.
- The second assumption, the random effect, the customer ID, comes from a Normal distribution, would be hard to assess, which will be discussed in the Strength and Limitation section later. 
- We checked the residual plot with respect to the fitted values and find no obvious pattern, so the third assumption holds: the errors from random effect and within-unit residual errors have constant variance.
- Since the number of quality flags for a sleep session, which is our response, is a count data, the response distribution we have chosen is the Poisson distribution. The log function is the link function we used when fitting the model, which would be appropriate for the Poisson distribution response variable. The fourth assumption is satisfied.

Now, we could proceed to discuss more about the estimated coefficients from the final model.

```{r, echo=FALSE, include=TRUE, results='asis'}
knitr::kable(summary(mod2)$coefficients[, c(1, 4)],
             caption = "Key Numerical Outputs of the Fixed Effects of the
             Reduced Model", col.names = c("Estimate", "P-value"))
```


Table 6 summarizes the estimated coefficient and p-value for each fixed effect of the reduced model. Similarly, since these p-values are all below the threshold, then we would reject the null hypothesis and choose to leave all predictors in the model as they are all related to the response.
...
rownames(exp_ci_df) = c("(Intercept)", "skinColorLight", "skinColorMedium", "skinColorMediumDark", "skinColorMediumLight", "skinColorUnspecified", "ageScale")
knitr::kable(exp_ci_df, caption = "95 percent Exponential CI in Final Model") %>% kable_styling(latex_options = "HOLD_position")
```

Table 7 summarizes the exponential of the 95%-confidence interval for each $\beta$ parameter. 95%-confidence interval means that we are 95% confident that each of the exponentials of $\beta$ parameters would be between the interval of the above lower bounds and upper bounds, respectively. For example, for exponential of $\beta_1$, the 95%-confidence interval is (`r exp_ci[3,1]`, `r exp_ci[3,2]`). It means that we are 95% certain that the number of quality flags during a sleep session for a customer with light skin color would be `r (exp_ci[3,1])*100`% to `r (exp_ci[3,2])*100`% times as many as dark skin color customer while keeping everything else constant. For the exponential of $\hat\beta_6$, the 95%-confidence interval is (`r exp_ci[8,1]`, `r exp_ci[8,2]`). When interpreting using age, it means that we are 95% certain that when the customer's age increases by 1, the count of the number of quality flags would occur during a sleep session decreases by `r (1-exp((ci[8,1])*(1/(92-17))))*100`% to `r (1-exp((ci[8,2])*(1/(92-17))))*100`% times while keeping everything else fixed. Basically, these 95%-confidence intervals are small for these exponentials of the $\beta$ parameter, which means that the estimated coefficients we fitted for the final model have high precision and the final model would be appropriate. 



Recall that the research question is to find out how the users’ skin color or other factors affect the performance of their devices’ sleep score function. After finishing the model selection steps, the final fitted generalized linear mixed model we fitted suggests that both the users’ skin color and their age are two main factors that would affect their devices’ sleep score function. 

By summarizing and interpreting the exact values of the estimated coefficients for the variable of user’s skin color, the average number of quality flags that occurred during a user’s sleep session would be the highest for users with dark skin. To be specific, for users of the same age, the average number of quality flags that occurred during a medium-dark skinned user’s sleep session would be `r (1-exp(output_mod2$coeff[4,1]))*100`% less than the number of quality flags for a dark-skinned user. Similarly, the expected number of quality flags that occurred during a medium-skinned user’s sleep session, a medium-light skinned user’s sleep session, or a light-skinned user’s sleep session would be `r (1-exp(output_mod2$coeff[3,1]))*100`%, `r (1-exp(output_mod2$coeff[5,1]))*100`%, or `r (1-exp(output_mod2$coeff[2,1]))*100`% less than the number of quality flags for a dark-skinned user, respectively. For each of the five levels of user’s skin color, since they are all compared with the same base level, darker-skinned users, then it is appropriate to conclude that the quality flag would occur more frequently for users with darker skin. For the group of users whose skin color is unspecified, it is not useful to draw any conclusion by its estimated coefficient ($\hat\beta_5$) as we do not have enough information, which has been explained in Data section. 

For the variable of a user’s scaled age, since it can be converted back to that user’s original age, then we could use it to conclude the relationship between users’ age and the number of sleep flags during their sleep session. Then, according to its estimated coefficient value, with a one-year-old increase in age, the average number of quality flags would decrease by `r (1-exp((output_mod2$coeff[7,1])*(1/(92-17))))*100`%, while for users with the same skin color. Thus, the quality flag during the sleep session would occur more frequently for younger users.

Therefore, the occurrence of the quality flag would be higher for younger users whose skin color is darker. It is reasonable to conclude that the user’s skin color would have a negative effect on the performance of the functionality of the device’s sleep score, and for a younger, darker-skinned user, his/her device would perform poorly for sleep score function. 
...
For the second research question, we are interested in whether darker skin color for customers affects the performance of the device’s sleep score function, or there are some additional effects that will influence the sleep score. We firstly constructed several graphical and numerical summaries to demonstrate the relationship between the number of occurrences of the quality flags during the sleep session and the customer’s skin color, other related variables. Then, we built a generalized linear mixed model with the number of flags that occurred during the sleep session as the response and three predictors (customers' skin color, customers' age, and type of device) and each customer as the random effect. We applied model selection techniques and chose the reduced model with customers’ skin color and age as the final model. In conclusion, the overall trend presents that darker-skinned users are more likely to occur quality flags during sleep sessions. Simultaneously, customers’ age would have some effect on the sleep score function, but it is not significant. Research states that it is more difficult to obtain accurate data since melanin-rich skin blocks green light which some device companies will use in their sensor[12]. Therefore, the conclusions we use the model to analyze are reasonable and it is consistent with those in the research.  
...
When fitting the generalized linear mixed model to find the difference between the Active/Advance buyers and traditional buyers, the model suffers from three limitations.
...
- When we cleaned the data, we noticed that there are multiple Census SubDivision Unique Identifier for one postal code, and we only keep the first record for each postal code. The downside of this decision is that our sample may be less representative and may introduce bias to the result of the model.
...
For the strengths, firstly, in the Data section, by using various types of figures and summarized tables, we explored the customers’ skin color and other variables of interest from the sleep dataset to discover the variables that would be correlated with the occurrence of quality flags during the sleep session. Secondly, in the Method and Result sections, the choice of a generalized linear mixed model with Poisson distribution for the response variable is appropriate. Thirdly, according to Table 6, the p-values of the estimated coefficients in the final model are relatively small, which reveals that it is appropriate and reasonable to include these predictors in the final model from a statistical perspective.  

There are three limitations shown as follows. 

- In the Data section, we created a new variable about the user's skin color by transforming the feature emojis of skin tone when using chat and react features of the social component of the app for the device. One of the categories of the variable is defined as “Unspecified” because the user did not specifically set an emoji modifier and uses the default state directly. Almost a quarter of the observations from the sleep dataset (5084/20181 = 25.19%) is for users with unspecified skin color, so we chose not to remove them. As a result, some users' information is not successfully obtained, which may affect the model selection and conclusions. In addition, the user may have a preference for the emoji modifier, so there may be some bias in inferring the user's skin color from the emoji modifier.
- Although the final model from the Result Section includes the user’s scaled age as one of the predictors, according to the interpretation of its exponential estimated coefficient ($e^{\hat\beta_6}$), when the customer's age increases by one, the average number of quality flags would occur during a user’s sleep session decreases by an extremely small percentage change (`r (1-exp((output_mod2$coeff[7,1])*(1/(92-17))))*100`%), while keeping everything else the same. This reveals that the effect of users’ age on the number of quality flags during their sleep session was not significant even if its p-value is smaller than the threshold. One possible reason may be that when fitting the model at first, for computational convenience, we chose to use the user’s rescaled age, which ranges from 0 to 1. This may affect the resulting value of the estimated coefficient. Further analysis on whether to include the predictor of the user’s scaled age for the model or not may be conducted in future. 
- When checking the second assumption of the fina model in the Result section, due to the limitation of the scope, it is hard for us to assess it in the current stage. For the third assumption, there is actually some pattern for small fitted values. It may be necessary to further investigate it in future. Besides, since the model we chose is a generalized linear mixed model with Poisson distribution for the response variable, then it would be better to check overdispersion. This may be another limitation and we may use some other external packages to check overdispersion to decide if we need to make some changes to the model in future. 
